...
class SIPRegisterHandler(socketserver.DatagramRequestHandler):
    ...
    def deleteUsers(self):
        ...
        for name in self.Users:
            ...
            Expiration = self.ExpiresTime(RegisteredTime,
                                          self.Users[name]['expires'])
            print(Expiration)
            if Expiration < Now:
    ...
    def ExpiresTime(self, registered, toexpire):
        ...
        RegistInSeconds = time.mktime(time.strptime(registered,
                                                    '%Y-%m-%d %H:%M:%S'))
...
AvailableMethods = ['REGISTER', 'INVITE', 'ACK', 'BYE']
